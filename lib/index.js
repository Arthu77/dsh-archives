...
export function apply(ctx, config) {
  ...
  async function deleteSession(id) {
    ...
    // Refuse only when a live agent is attached (running or awaiting input).
    // The in-memory session store keeps every entered session until its owning
    // fiber dies — archiving merely hides the row and does NOT destroy that
    // instance — so `sessions.get(id)` is far too strict a proxy for "in use".
    const agentsSvc = ctx.get('agents')
    if (agentsSvc && typeof agentsSvc.get === 'function' && agentsSvc.get(id)) {
      throw new Error('该会话仍有活跃任务（正在运行或等待输入），请先停止它再删除')
    }
